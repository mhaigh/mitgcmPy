...
if MAIN:
	...
	np.save('slope_xi', slope_xi); np.save('slope_yi', slope_yi)
	...
	slope_uv_av = np.zeros((nT, nX))

	slope_uv_max = np.zeros((nT, nX))

	ufile = 'UVEL'; vfile = 'VVEL';	rhofile = 'RHOAnoma'

	for ti in range(1):

		for xi in range(50,nX):
			...
			slope_uv_av[ti,xi] = np.ma.mean(slope_uv)

			slope_uv_max[ti,xi] = np.ma.max(slope_uv)
