...
if animBin:
	...
	data = readnp(path2+VAR, dims, rec=0, dtype='>f8')
...
if TEST_animate:
	...
	if not MEAN:
		...
		data = np.mean(data[...,100:140], axis=-1)
	...
	#pt.plot1by1(datam, X, Y, xlabel=xlabel, ylabel=ylabel, title=title, cmap=cmap, vmin=vmin, vmax=vmax, mesh=False); quit()
...
if animateSurface:
	...
	if VAR == 'npy':
		...
		#ones = np.cumsum(np.ones(data.shape[0]))
		...
		#			data[:,j,i] = PAS_tools.detrend(data[:,j,i], ones)
		...
		data = readVariable('THETA', path, file_format='nc', meta=True)

		text_data = ptt.getTextData(data.variables['TIME'][:], 'month', X[1], Y[1], color='k')

	

		data = data.variables['THETA'][:][-1]

		#data = tools.getIsothermHeight4(data[-1], grid.RC.squeeze(), Y, X, -0.5)

		

		ThermZ = tools.getIsothermDepth3(data, Z, -0.5)		

		

		pt.plot1by1(ThermZ, vmin=-600, vmax=-150)
	...
	#vmin = -0.002; vmax = -vmin # For SSS

	

	#plt.plot(np.mean(tools.ddy(data[-12:,1:-1,:]),axis=(0,-1)), Y[1:-1]); plt.grid(); plt.title('Zonal mean wind stress curl'); plt.ylabel('Y (km)'); plt.show(); quit()

	#data = tools.ddy(data, dy=1.); vmin = -4.e-4; vmax = -vmin

	

	pt.animate1by1(data, X, Y, cmap=cmap, xlabel=xlabel, ylabel=ylabel, title=title, mesh=False, text_data=text_data, outname='animate1by1Surf.mp4', vmin=vmin, vmax=vmax, contour=bathy, contourfNlevels=21, figsize=figsize)
...
plotSurfaceStresses = False

if plotSurfaceStresses:
	...
	runs = ['PISOMIP_003', 'PISOMIP_004']

	labels = ['tauu IPO neg', 'tauu IPO pos']
	...
	for ri, run in enumerate(runs):			
		...
		data = readVariable(VAR, path, file_format='nc', meta=False)

		print(data.shape)

		

		plt.plot(np.mean(data[-12:],axis=(0,-1)), Y, color=colors[ri], label=labels[ri]); 

	

	plt.legend()

	plt.grid(); plt.title('Zonal mean wind stress'); plt.ylabel('Y (km)')
